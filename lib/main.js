...
function getUrlFromGuid(runtag, feedurl, entry) {
  if (feedurl == 'http://blog.vivekhaldar.com/rss')
    log(runtag, "trying to get url from guid");
  try {
    var guid = entry.getElementsByTagName('guid')[0].innerHTML;
    if (feedurl == 'http://blog.vivekhaldar.com/rss')
      log(runtag, "guid is "+typeof(guid)+": "+guid);
    ios.newURI(guid, null, null);
    console.log("guid is url: "+guid);
    return guid;
  } catch(e) {
    if (feedurl == 'http://blog.vivekhaldar.com/rss') {
      log(runtag, "guid is not url");
      log(runtag, e);
    }
  }
}
...
function getGuid(entry) {
  try {
    return entry.getElementsByTagName('guid')[0].innerHTML;
  } catch (anything) {
  }
}
