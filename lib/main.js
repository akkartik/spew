...
function addNewItems(runtag, feedurl, body) {
  var urls = feedItems(runtag, parser.parseFromString(body, "application/xml"));
...
function feedItems(runtag, feed) {
  log(runtag, "feedItems");
  var result = [];
  // RSS
  log(runtag, "aa: "+feed.childNodes[0]+" "+feed.childNodes[1].innerHTML);
  ...
  for (var i = 0; i < entries.length; ++i) {
    ...
    if (links.length > 0)
      result.push(links[0].getAttribute('href'));
  }
