...
function crawlSomeFeeds(runtag) {
  ...
  log(runtag, "about to crawl "+currentlyCrawlingIndex+"/"+folderNode.childCount);
...
menu.Item({
  ...
  onMessage: function() {
    log("interactive", "menu clicked");
    addFeed("http://akkartik.name/feed");
  }
});
...
var logStream = file.open("/tmp/spew.log", "w");
